...
> Чат один и тот же для метрик/ошибок/отчётов — различаются только топики внутри него.
...
Один чат, топики настраиваются при установке. По умолчанию Метрики и Критика используют **один и тот же топик** (`<ID_ОШИБОК>`), Отчёты — отдельный (`<ID_ОТЧЁТОВ>`). При желании можно развести Метрики на свой топик — спросится отдельно при установке.
