...
6. Создаёт реальный Host (флаг+страна, fingerprint firefox, привязан к
   уже существующему инбаунду ноды, помечен **невидимым** — `isHidden:
   true` — клиент его напрямую не выбирает, он только цель для
   tagRegex-инжекта из авто-хостов) и отдельный виртуальный
   хост-держатель для клиентского RU-обхода (`<страна> (device-route
   RU)`, **видимый**, на общем config-profile `VirtualHost` — тоже уже
   существующем, не создаётся заново), плюс JSON-шаблон подписки для него.
