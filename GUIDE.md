...
0. Первым делом проверяет, существуют ли два общих клиентских авто-хоста
   балансировщика — **Auto ДЛЯ ДОМАШНЕГО ИНТЕРНЕТА** (тег
   `BALANCER_WIFI`) и **Auto ДЛЯ МОБИЛЬНОГО ИНТЕРНЕТА BRIDGE** (тег
   `BALANCER_MOBILE`). Именно к ним подключается клиент — через
   `tagRegex`-инжект в них подставляется любая живая нода с нужным тегом.
   Если хоста ещё нет — создаёт его (на общем профиле `VirtualHost`) и
   привязывает JSON-шаблон. Идемпотентно: если уже есть — просто пропускает.
...
6. Создаёт два хоста на одном и том же инбаунде ноды:
   - **реальный хост-держатель тега** (`<страна> (VH)`, **скрыт** —
     `isHidden: true`) — на нём balancer.py проставляет тег пула по
     здоровью, но клиент его не видит и не выбирает;
   - **видимый хост** (`<страна>`, **не скрыт**) — именно его выбирает
     клиент, и именно на него привязан JSON-шаблон подписки
     (`build_subscription_template`) с полным RU-bypass роутингом
     (`domain:ru`, `vk.com`, `yandex.*`, `mail.ru`, ... → direct).

   Порядок специально такой, а не наоборот: self-инъекция в Remnawave
   запрещена (хост не может нести шаблон, который инжектит сам себя по
   UUID) — если попытаться повесить шаблон на реальный хост напрямую,
   маршрутизация в шаблоне применится, но пропадёт сам outbound `proxy`
   (VLESS-туннель), и подключение сломается для всех, кто выберет этот
   сервер. Поэтому шаблон обязан жить на ОТДЕЛЬНОМ (видимом) объекте,
   который инжектит реальный хост по UUID — а не наоборот. Если этот
   порядок случайно перепутать (например, руками в панели), результат:
   выбранный сервер либо вообще без RU-bypass (голый хост без шаблона),
   либо с рабочим роутингом, но без туннеля (self-инъекция). Проверено
   вживую 2026-07 на продакшене — именно так и обнаружилась эта ошибка.
7. Регистрирует ноду в `balancer.py` — как обычно в `add_node.py`. Сам
   тег (`BALANCER_WIFI`/`BALANCER_MOBILE`) на реальном хосте проставляет
   уже работающий балансировщик по результатам проверки здоровья, не
   этот скрипт напрямую.
