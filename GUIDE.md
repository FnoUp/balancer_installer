...
Пункт меню `balancer` → **4) Автодобавление ноды**. В отличие от
`add_node.py` (ручной ввод IP/UUID), этот пункт вообще не требует данных
о ноде на входе — он сам:

1. Показывает список нод, уже подключённых к Remnawave (`GET /api/nodes`)
   — просто выбираешь номер, IP/UUID вводить не нужно.
2. Проверяет SSH-доступ с панели на ноду (`root@<адрес ноды>`) — если
   ...
   ничего не поменяв.
3. Спрашивает только то, что нельзя вывести автоматически: тип ноды
   (простая / мост-вход / мост-выход) и пул (BALANCER / BALANCER_WIFI /
   BALANCER_MOBILE).
4. Сам определяет сетевой интерфейс ноды: резолвит домен ноды через DNS,
...
5. Генерирует Reality-ключи прямо на ноде (`xray x25519` через Docker).
6. Собирает config-profile (по образцу `Templates/Node_*.txt`), создаёт
   его в Remnawave и привязывает к ноде — единственный шаг, который
   явно показывает точный запрос перед выполнением и просит отдельное
   подтверждение.
7. Создаёт реальный Host (флаг+страна, fingerprint firefox) и отдельный
   виртуальный хост-держатель для клиентского RU-обхода (`<страна>
   (device-route RU)`, по образцу существующих Finland/Sweden/Moscow),
   плюс JSON-шаблон подписки для него.
8. Регистрирует ноду в `balancer.py` — как обычно в `add_node.py`.
