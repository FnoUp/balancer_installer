...
# ── Интерактивные вопросы (переиспользуются auto_add_node.py) ──
def ask_net_device():
    print()
    ...
    if iface_input == "2":
        ...
        return ask("Введи интерфейс вручную", "eth0")
    else:
        return "eth0"
...
def ask_pool_tag():
    ...
    return {"1": "BALANCER", "2": "BALANCER_WIFI", "3": "BALANCER_MOBILE"}[pool_input]
...
# ── Применение к конфигам (переиспользуется auto_add_node.py) ──
def apply_node_to_configs(node_name, node_ip, location, net_dev, host_uuid, pool_tag, tg_name):
    """Всё, что происходит после подтверждения: запись конфигов Prometheus,
    вставка в NODES balancer.py, рестарт сервисов, TG-уведомление."""
...
if __name__ == "__main__":
    ...
    apply_node_to_configs(node_name, node_ip, location, net_dev, host_uuid, pool_tag, tg_name)
